      ...
      for (i in 0..<samples.items.size) {
        ...
        try {
          ...
          mediaFile.second?.let { uri ->
            samplesRepository.downloadFile(item.url).collectIndexed { index, inputStream ->
          ...
          // Clean up
          mediaFile?.second?.let { galleryRepository.setMediaFileDeleted(it) }
          galleryRepository.deleteSampleMediaSubFolder(relativePath)
          // Error and stop
          _state.value = UiSamplesState.DownloadError(e.message ?: "Failed to download samples")
          return@launch
        }
